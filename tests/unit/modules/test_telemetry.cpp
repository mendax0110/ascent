...
TEST_F(DataLoggerTest, LogData)
{
    ...
    auto status = logger.log(data, sizeof(data));
    EXPECT_EQ(status, StatusCode::Ok);
...
TEST(PacketFormatter, FormatStatusDoesNotCrash)
{
    ...
    uint8_t buffer[256] = {};
    size_t written = 0;
    auto status = fmt.formatStatus(buffer, sizeof(buffer), written);
    EXPECT_EQ(status, StatusCode::Ok);
}
...
TEST(PacketFormatter, FormatNavigationDoesNotCrash)
{
    ...
    uint8_t buffer[256] = {};
    size_t written = 0;
    auto status = fmt.formatNavigation(buffer, sizeof(buffer), written);
    EXPECT_EQ(status, StatusCode::Ok);
}
